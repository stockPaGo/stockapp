 ...
 * 종목별 주식  조회
 * */
class CrawlingEachDAY {
  getPer () => fetchPhotos();

  Future<List<DAY>> fetchPhotos() async {
    ...
    print('http.Response response = await http.get');
    ...
    print('http.Response response2 = await http.post');

    // compute 함수를 사용하여 parsePhotos를 별도 isolate에서 수행합니다.
    return parsePhotos(utf8.decode(response2.bodyBytes));
  }

  List<DAY> parsePhotos(String responseBody) {
    print('List<DAY> parsePhotos(String responseBody)');

    final parsed = json.decode(responseBody)['block1'].cast<Map<String, dynamic>>();
